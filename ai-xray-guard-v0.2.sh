...
log_error() {
    echo -e "${RED}$(date '+%Y-%m-%d %H:%M:%S') [ERROR] $1${PLAIN}" | tee -a "$LOG"
}

log_warn() {
    echo -e "${YELLOW}$(date '+%Y-%m-%d %H:%M:%S') [WARN] $1${PLAIN}" | tee -a "$LOG"
}

log_ok() {
    echo -e "${GREEN}$(date '+%Y-%m-%d %H:%M:%S') [OK] $1${PLAIN}" | tee -a "$LOG"
}
...
get_private_key() {
    if [ -f "$REALITY_KEY" ]; then
        grep "^private:" "$REALITY_KEY" | cut -d: -f2 | tr -d ' '
    else
        jq -r '.inbounds[0].streamSettings.realitySettings.privateKey' "$CONFIG" 2>/dev/null || echo ""
    fi
}

get_public_key() {
    if [ -f "$REALITY_KEY" ]; then
        grep "^public:" "$REALITY_KEY" | cut -d: -f2 | tr -d ' '
    else
        # Derive from private key
        local priv=$(get_private_key)
        if [ -n "$priv" ]; then
            xray x25519 -i "$priv" 2>/dev/null | grep "Public" | awk '{print $NF}'
        fi
    fi
}
...
count_rst() {
    # Count TCP connections in abnormal states on port 443
...
can_rotate() {
    ...
    if [ $elapsed -lt $ROTATION_COOLDOWN ]; then
        local remaining=$(( ROTATION_COOLDOWN - elapsed ))
        log_warn "Rotation cooldown active (${remaining}s remaining)"
        return 1
    fi
...
rotate_dest() {
    ...
    # Generate new shortId (keep empty string for compatibility)
    ...
    # Restart Xray - this WILL disconnect clients
    log_warn "Restarting Xray (clients will disconnect briefly)..."
    ...
    log "[MORPH] $old_dest -> $new_dest (reason: $reason, shortId: $old_sid -> $new_sid)"
    return 0
}
...
main() {
    init_db
    log "[START] AI Guard v0.2 started (threshold-based rule engine)"
    log "[INFO] Rotation triggers restart - clients will disconnect briefly"
    log "[INFO] Thresholds: latency>${LATENCY_THRESHOLD}ms, loss>${LOSS_THRESHOLD}%, rst>${RST_THRESHOLD}, consecutive=${CONSECUTIVE_FAILURES}"

    while true; do
        local dest=$(get_current_dest)
        
        if [ -z "$dest" ]; then
            log_error "Cannot read dest from config, sleeping..."
        ...
        if [ "$status" = "degraded" ]; then
            ...
            log_warn "$reason (consecutive: $consecutive/$CONSECUTIVE_FAILURES)"
            
            if [ $consecutive -ge $CONSECUTIVE_FAILURES ]; then
                log_warn "Threshold reached, triggering rotation..."
                if rotate_dest "$reason"; then
                    log_ok "Rotation complete"
                else
                    log_error "Rotation failed or in cooldown"
                fi
            fi
        else
            if [ $consecutive -gt 0 ]; then
                log_ok "Health recovered (was: $consecutive consecutive failures)"
            fi
            set_meta "consecutive_failures" "0"
            log_ok "dest=$dest latency=${CURRENT_LATENCY}ms loss=${CURRENT_LOSS}% rst=$CURRENT_RST conn=$conn_info"
        fi
