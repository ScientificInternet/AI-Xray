...
指标超过阈值 = "GFW 在看我了"。不等到被封，提前行动。

### 换脸

修改 Xray 配置中的 dest / SNI / shortId，发送 SIGHUP 信号热重载。用户无感，连接不断。同一台 VPS，身份不断变。

### 记录

每次换脸的结果写入本地 SQLite：哪个 dest 活了多久、什么时段最安全、什么触发了换脸。数据越积越多，决策越来越准。

### 流量整形

控制并发连接数和速率曲线，让流量行为看起来像正常网站访问，不像代理。新安装的节点流量逐步增长，不会突然暴涨引起注意。
...
- 客户端（OpenClash 插件）**零数据上传**
- 服务端 AI 守卫的所有决策在本地完成
- 换脸日志仅存本地 SQLite，不外传
- opt-in 网络质量数据通过 M-Lab 公开平台上报（完全匿名、自愿）
